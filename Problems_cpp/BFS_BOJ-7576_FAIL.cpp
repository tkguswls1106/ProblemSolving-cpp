...
// 이는 방문체크를 통해 안가도되는 칸을 걸러도 되는것이 아니었기에, 중복되어 방문할수있다. 즉, 최악의 경우인 '익은토마토가 전체 칸수 모두'일경우에는 n*m만큼 더 중복되어 돌릴수있다는 것이다
// => BFS의 시간복잡도가 O(n * m)이지만, 최악의 경우에는 익은 토마토 또한 최대 n*m개가 있을 수 있으니 총 O(n^2 * m^2)이 되어 시간 내로 해결이 안될것이기 때문이다.
// - 해결 방법:
// 초반에 익은 토마토 좌표들을 전역변수로 선언한 큐에 먼저 push넣어주고 그대로 진행하면 된다.
// 즉, board[i][j]==1 일때 qu.push({i,j})를 먼저 해주면 된다는 것이다.
...
void bfs(int x, int y) {
    if(board[x][y] == -1) return;  // 비어있는 토마토자리는 건너뜀. (사실 이 코드줄은 어차피 첫좌표를 1값을 주기때문에 안적어도된다.)

    queue<pair<int, int>> qu;
    qu.push({x,y});
    dayV[x][y] = 0;  // 이 문제의 경우에는 익는데걸리는 일수를 0부터 스타트해야함.
...
int main()
{
    ...
    vector<pair<int, int>> v;  // 처음부터 이미 익어있었던 1토마토들의 좌표를 담을 배열
    ...
    for(auto p : v) {
        bfs(p.first, p.second);  // 처음부터 이미 익어있었던 1토마토들의 좌표부터 시작하는 bfs 함수를 반복문으로 호출.
    }
