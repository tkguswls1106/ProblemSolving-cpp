...
// * 일반적인 큐와는 다르게, 우선순위큐는 qu.front() 말고 pq.top()을 사용한다.
...
struct cmp {
	bool operator() (int n1, int n2) {
        if(abs(n1) != abs(n2)) {
            ...
            // 절댓값이 작은 것에 우선순위를 높게 주겠다는 의미. (일반적인 커스텀 sort와 반대로 생각할것! 주의!!)
        }
        else {  // else if(abs(n1) == abs(n2))
            return n1 > n2;
            // 실제값기준 오름차순 (실제값기준 내림차순의 반대.)
            // 절댓값이 같다면, 원래의 실제값이 더 작은 것에 우선순위를 높게 주겠다는 의미. (일반적인 커스텀 sort와 반대로 생각할것! 주의!!)
        }
    }
};
